...
public class ProjectileMoveScript : Projectile
{
        ...
        if (!bounce)
        {
            if (co.gameObject.tag != "Bullet" && !collided)
            {
                ...
                if (co.collider.gameObject.tag == "Player")
                {
                    ...
                    //Its coded this way so that if multiple projectiles that hit a player at the same time (IE from a shotgun),
                    //it doesn't increase their score by an excessive amount
